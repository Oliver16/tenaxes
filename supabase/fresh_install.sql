...
CREATE TABLE public.question_bank_versions (
    ...
    CONSTRAINT question_bank_versions_status_check CHECK ((status = ANY (ARRAY['draft'::text, 'published'::text, 'archived'::text])))
);
...
INSERT INTO public.question_bank_versions (id, name, notes, question_count, collision_count, status) VALUES
    ('v2.2', 'Polyaxis v2.2 — Controversy Stress Test', '18 constructs; 350 questions; v2.1 comprehension treatment plus 50 overt high-conflict items; 48 collision scenarios across 24 mirrored pairs.', 350, 48, 'published');
...
-- Keep question-bank revisions internally consistent and support atomic cloning.
...
AS $$
  ...
  SET question_count = (
      ...
      collision_count = (
        ...
        WHERE question.bank_version = p_bank_version AND link.role = 'tradeoff'
      )
