...
export function validateAnalysisEvidence(analysis: PersonalizedAnalysis, input: PersonalizedAnalysisInput): EvidenceValidationResult {
  ...
  analysis.tensions.forEach((item, i) => {
    ...
    if (item.classification === 'possible_hypocrisy') {
      ...
      if (stronglyEndorsedPrinciples.length < 1 || applied.length < 2 || item.confidence === 'low') {
  ...
  analysis.blind_spots.forEach((item, i) => checkQuestions(item.question_ids, `blind_spots[${i}]`))
  const tensionIds = new Set(analysis.tensions.map(t => t.tension_id))
  analysis.clarifying_questions.forEach((item, i) => item.related_tension_ids.forEach(id => {
    if (!tensionIds.has(id)) errors.push(`clarifying_questions[${i}]: nonexistent tension ${id}`)
  }))
